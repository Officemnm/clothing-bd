...
export default function ChallanPreviewPage() {
  ...
  const handleScreenshot = async () => {
    ...
    try {
      ...
      link.download = `Challan-Report-${currentDate}.png`;
      link.href = dataUrl;
      link.click();
    } catch (error) {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      pdf.save(`Challan-Report-${currentDate}.pdf`);
    } catch (error) {
  ...
  // Calculate summary by buyer
  const buyerSummary = reportData.data.reduce((acc, item) => {
    if (!acc[item.buyer]) {
      acc[item.buyer] = 0;
    }
    acc[item.buyer] += item.totalQty;
    return acc;
        ...
        <div className="flex justify-center">
          <div ref={reportRef} className="w-full max-w-4xl bg-white rounded shadow-sm overflow-hidden">
            ...
            <div className="mx-3 my-2 grid grid-cols-2 gap-4 border-b border-black pb-2">
              ...
              {/* Right: Buyer-wise Summary */}
              <div>
                <h3 className="text-xs font-bold text-black uppercase mb-1 border-b border-black pb-0.5">Buyer Wise</h3>
                <div className="space-y-0">
                  {Object.entries(buyerSummary).map(([buyer, qty], idx) => (
                    <div key={idx} className="flex justify-between text-xs">
                      <span className="text-black truncate max-w-[140px]">{buyer}</span>
                      <span className="text-black font-bold">{qty.toLocaleString()}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* Main Data Table */}
            <div className="p-2">
              <div className="mb-3" style={{ border: '1px solid black', borderRadius: '2px', overflow: 'hidden' }}>
                {/* Table Header */}
                ...
                <table className="w-full" style={{ borderCollapse: 'collapse' }}>
                  <thead>
                    <tr className="bg-gray-200">
                      ...
                      <th className="text-right font-bold text-black text-sm bg-gray-300" style={{ border: '1px solid black', padding: '4px 6px', width: '100px' }}>QTY</th>
                    </tr>
                  </thead>
                  <tbody>
                    {reportData.data.map((item, index) => (
                      <tr key={item.challanNo} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                        ...
                        <td className="text-black text-sm font-medium" style={{ border: '1px solid black', padding: '4px 6px', color: '#6d28d9' }}>{item.challanNo}</td>
                        ...
                        <td className="text-right text-black text-sm font-medium bg-gray-100" style={{ border: '1px solid black', padding: '4px 6px' }}>{item.totalQty.toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot>
                    <tr className="bg-green-100">
                      <td colSpan={5} className="text-right font-bold text-black text-sm" style={{ border: '1px solid black', padding: '4px 6px' }}>GRAND TOTAL</td>
                      <td className="text-right font-bold text-black text-base bg-green-200" style={{ border: '1px solid black', padding: '4px 6px' }}>{reportData.grandTotal.toLocaleString()}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
