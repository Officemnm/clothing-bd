                ...
                <table className="w-full" style={{ borderCollapse: 'collapse' }}>
                  <thead>
                    <tr className="bg-gray-200">
                      <th className="text-left font-bold text-black text-sm whitespace-nowrap" style={{ border: '0.5px solid black', padding: '0px 3px', width: 'auto' }}>PO No</th>
                      {table.sizes.map((size, i) => (
                        <th key={i} className="text-center font-bold text-black bg-gray-200 text-sm" style={{ border: '0.5px solid black', padding: '0px 2px' }}>{size}</th>
                      ))}
                  ...
                  <tbody>
                    ...
                    {table.rows.map((row, rowIdx) => (
                      <tr key={rowIdx}>
                        <td className="font-bold text-black text-sm whitespace-nowrap" style={{ border: '0.5px solid black', padding: '0px 3px' }}>{row.poNo}</td>
                        {row.quantities.map((qty, qtyIdx) => (
                          <td key={qtyIdx} className={`text-center font-semibold text-sm ${qty === 0 ? 'text-gray-400' : 'text-black'}`} style={{ border: '0.5px solid black', padding: '0px 2px' }}>
                            {qty === 0 ? '-' : qty}
                          </td>
                        ))}
                        <td className="text-center font-bold text-black bg-gray-50 text-sm" style={{ border: '0.5px solid black', padding: '0px 3px' }}>{row.total}</td>
                      </tr>
                    ))}
                    {/* Actual Qty - Highlighted */}
                    <tr className="bg-green-100">
                      <td className="font-bold text-black bg-green-100 text-sm whitespace-nowrap" style={{ border: '0.5px solid black', padding: '0px 3px' }}>Actual Qty</td>
                      {table.actualQty.map((qty, i) => (
                        <td key={i} className="text-center font-bold text-black bg-green-100 text-sm" style={{ border: '0.5px solid black', padding: '0px 2px' }}>{qty}</td>
                      ))}
                      <td className="text-center font-bold text-black bg-green-100 text-sm" style={{ border: '0.5px solid black', padding: '0px 3px' }}>{table.actualTotal}</td>
                    </tr>
                    {/* Order Qty 3% - Highlighted */}
                    <tr className="bg-amber-100">
                      <td className="font-bold text-black whitespace-nowrap bg-amber-100 text-sm" style={{ border: '0.5px solid black', padding: '0px 3px' }}>Ord Qty 3%</td>
                      {table.orderQty3Percent.map((qty, i) => (
                        <td key={i} className="text-center font-bold text-black bg-amber-100 text-sm" style={{ border: '0.5px solid black', padding: '0px 2px' }}>{qty}</td>
                      ))}
                      <td className="text-center font-bold text-black bg-amber-100 text-sm" style={{ border: '0.5px solid black', padding: '0px 3px' }}>{table.orderTotal}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
