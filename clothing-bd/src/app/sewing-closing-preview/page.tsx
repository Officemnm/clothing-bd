...
export default function SewingClosingPreviewPage() {
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem('sewingClosingReportData');
    if (stored) {
  ...
  const handleScreenshot = async () => {
    ...
    try {
      ...
      link.download = `Sewing-Closing-Report-${reportData.refNo}-${currentDate}.png`;
      link.href = dataUrl;
      link.click();
    } catch (error) {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      pdf.save(`Sewing-Closing-Report-${reportData.refNo}-${currentDate}.pdf`);
    } catch (error) {
      ...
      <div className="min-h-screen bg-gray-50 p-2 font-sans">
        ...
        <div className="no-print max-w-6xl mx-auto mb-3 flex justify-end gap-3 p-3 bg-white rounded-xl shadow-md border border-gray-100">
          ...
          <button 
            ...
            title="Screenshot"
            className="p-2.5 text-white bg-gradient-to-b from-rose-500 to-rose-700 rounded-lg hover:from-rose-600 hover:to-rose-800 hover:shadow-md transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isCapturing ? (
          ...
          <div ref={reportRef} className="w-full max-w-6xl bg-white rounded shadow-sm overflow-hidden">
            ...
            <div className="flex justify-between items-center px-4 py-3 border-b border-gray-300 bg-gray-50">
              <div className="flex gap-4">
                <div className="bg-rose-50 border border-rose-200 rounded-lg px-4 py-2 shadow-sm">
                ...
                {reportData.meta.item && reportData.meta.item !== 'N/A' && (
                  <div className="bg-rose-50 border border-rose-200 rounded-lg px-4 py-2 shadow-sm">
                    <p className="text-[10px] text-black uppercase tracking-wide font-medium">Item</p>
                    <p className="text-sm text-black font-bold">{reportData.meta.item}</p>
                  </div>
                )}
              </div>
              <div className="bg-rose-50 border border-rose-200 rounded-lg px-4 py-2 shadow-sm text-right">
            ...
            <div className="p-1">
              {reportData.data.map((block, idx) => (
                <div key={idx} className="mb-1" style={{ border: '1px solid black', borderRadius: '2px', overflow: 'hidden' }}>
                  {/* Color Header */}
                  <div className="px-1 text-sm font-bold uppercase text-black" style={{ backgroundColor: '#fecaca', borderBottom: '1px solid black', lineHeight: '1.3' }}>
                    {block.color}
                  </div>
